...
function widget:DrawWorld()
  ...
  ----------------------------------------------------------------------
  -- 1. Permanently rendered layout lines (from Render button)
  ----------------------------------------------------------------------
  if renderedLines and #renderedLines > 0 then
    gl.Color(1, 1, 0, 0.9)
    gl.LineWidth(2)
    gl.BeginEnd(GL.LINES, function()
      for _, ln in ipairs(renderedLines) do
        gl.Vertex(ln.startX, ln.y + 2, ln.startZ)
        gl.Vertex(ln.endX,   ln.y + 2, ln.endZ)
      end
    end)
  end

  ----------------------------------------------------------------------
  -- 2. Current layout being edited (green)
  ----------------------------------------------------------------------
  gl.Color(0, 1, 0, 0.7)
  gl.LineWidth(2)
  gl.BeginEnd(GL.LINES, function()
...
function widget:Update(dt)
  ...
  if renderTimer < 0.02 then return end
  renderTimer = 0
  ...
  -- move one line into a separate rendered list
  renderedLines = renderedLines or {}
  table.insert(renderedLines, table.remove(drawLineQueue, 1))
end
